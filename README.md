...
아직이면 아래 둘 중 하나를 고른다. **둘 다 RAM을 기다리지 않는다.**

### ㄱ. 2단계 리허설 — 다만 조율이 먼저다

개발 PC에서 DB를 인스턴스 둘로 합쳐 형태를 검증한다. 절차는 런북 01의 9절 그대로다.

⚠️ **착수 전에 반드시 확인할 것** — 이 작업은 세 저장소의 `DATABASE_URL`을 바꾼다.
지금 `RF_Service_System`과 `njlee`에서 **다른 세션이 활발히 작업 중**이다.
그쪽 개발이 멈추므로 사용자에게 먼저 묻는다. 막는 것이 기술이 아니라 일정이다.

### ㄴ. 6단계 사전 확인 — 사용자 답을 기다리는 일

공유기 모델명(VPN 지원 여부)과 회선 업로드 속도. 코드 작업이 아니라 질문이다.

---

## 인계 메모

- **A/S에 적용 대기 마이그레이션 1건** — `0078_blue_bullseye`.
  `npm run db:preflight`가 *"더하기만 합니다 — 사라지는 자료 없음"*이라고 답한다.
  **다른 세션의 작업이므로 이식 세션에서 적용하지 않는다.**
