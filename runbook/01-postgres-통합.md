...
### 9절 부록 2 — NAS 운영 전환 기록 (2026-09-14)

**끝났다.** 16:23~16:31, 중단 약 8분. 스크립트는 [`nas/setup/`](../nas/setup/), 결과 로그는 NAS `/volume1/dss/setup/logs/`.
검증 숫자는 README 「검증 — 2026-09-14」.

#### 이 절과 달랐던 것 — 다음에 이 절을 따를 사람이 알아야 한다

1. **4·5단계는 따로 할 수 없었다(6단계의 프록시, 7단계의 등록까지 함께다).** NAS 의 DB 는 포트를 열지 않으므로
   개발 PC 의 앱이 닿지 못하고, 앱도 포트를 열지 않으므로 프록시 없이는 아무도 닿지 못한다. 그래서 **NAS 에 전부를
   예행으로 먼저 세우고(`02-rehearse.sh`), 전환 날 앱을 멈춘 뒤 최종 덤프로 `--reset` 해 다시 부었다.**
2. **덤프 원천은 옛 상자가 아니라 `dss-pg-app` · `dss-pg-auth`** 다(2단계 이후). Phase 1 의 명령은 그대로 쓰지 않는다.
3. **A/S 표 구조는 `db:migrate` 가 아니라 `--schema-only` 덤프 + 뼈대 20장 · `drizzle.__drizzle_migrations`(98건) 자료 덤프**로
   넣었다. 도구 이미지가 아직 없어서다. 결과는 같다(98건 적용된 모양). 뼈대 자료는 자기참조 FK 때문에 관리자 롤로
   `--data-only --disable-triggers --single-transaction`.
4. **계측기 원천은 개발 PC 사본** — 08-28 14:23 백업과 핵심 표(계측기 79 · 교정 333 · 성적서 350 · 사진 113)가 같다.
   이남준 님 PC 의 자동 백업은 **08-28 아침부터 전부 0바이트**였다(PC 를 켜면 DB 가 꺼진 채 백업이 먼저 돈다).
   그 뒤 변경분은 옮기지 않았다 — 사용자 결정(CLAUDE.md G).
5. **포털 등록 주소는 NAS 사본에서 SQL 로** 바꿨다(02 의 5단계) — `{lan}` 을 `192.168.0.222` 로 바꾼 값을 더하고
   백채널 · 런처는 바꿨다. 개발 PC 의 로그인 DB 는 그대로다.
6. **업로드 폴더도 되돌려야 한다.** `--reset` 은 DB 만 지우므로 예행 때 올린 첨부 5개가 짝 잃은 파일로 남았다.
   02 에 되돌리기 단계를 넣었다 — A/S 첨부는 비우고, 계측기는 개발 PC 목록(`meters-files.manifest`)에 없는 것만 지운다.
7. **`dss` 공유 폴더의 시놀로지 ACL 은 DSM 계정만 안다.** 컨테이너 사용자(node 1000 · postgres 999)를 위해 하위 폴더의
   ACL 을 걷고 리눅스 권한으로 줬다(01). 부모 폴더의 ACL 은 바인드 마운트 안쪽 접근에 걸리지 않는다.
8. **NAS 의 docker 명령은 사람이 `sudo` 로** 돌렸다. Claude 는 swhur 열쇠로 파일을 올리고 로그를 읽었다(CLAUDE.md H).
9. **비밀번호 다섯을 전환 전에 새로 만들었다.** 이 세션에서 개발 PC 앱 비밀번호 셋이 화면에 찍혔고 `.env.nas` 와 같은
   값이었다. NAS 인스턴스를 만들기 전이라 비용이 없었다. 개발 PC 인스턴스는 옛 값 그대로다.
10. **DS218+ 의 DSM 은 7.2.2 가 끝이고, 7.1 의 Docker(20.10.3)는 쓰지 않았다** — bookworm 계열 이미지가 clone3 seccomp 에
    걸릴 수 있다. Container Manager 24.0.2 · Compose v2.20.1 이 이 파일(`name:` · `deploy.resources`)을 그대로 읽는다.
