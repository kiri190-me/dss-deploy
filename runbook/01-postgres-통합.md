...
### 결정 A — `dss_home`은 바깥 호스팅 ✅

회사 홈페이지의 운영 DB는 사내 NAS에 두지 않는다. NAS가 담는 DB는
`dss_auth` · `dss_as` · `dss_meters` 셋이다.

`dss-home/README.md:48`의 "NAS로 옮길 때"는 **앱** 이야기이지 DB가 아니다.
혼동을 막기 위해 그 문장에 단서를 다는 편이 좋다.
...
`njlee/REQUIREMENTS.md`의 미정 항목 **T4**가 여기서 해소된다. 롤마다 **서로 다른**
비밀번호를 쓴다 — 같은 값을 돌려쓰면 롤을 나눈 의미가 절반으로 준다.
